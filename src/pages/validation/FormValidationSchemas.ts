...
export const SignUpFormValidationSchema:Yup.SchemaOf<SignUpFormValues> =  Yup.object({
    //First step fields
    email: Yup.string().email('Se requiere un email válido.').required('Se requiere un email.').max(EMAIL_MAX_LENGTH,'El email no debe contener más de ' + EMAIL_MAX_LENGTH + ' caracteres.'),
    password: Yup.string().required('Se require una contraseña.').max(PASSWORD_MAX_LENGTH,'La contraseña no debe contener mas de ' + PASSWORD_MAX_LENGTH +' caracteres.').min(PASSWORD_MIN_LENGTH,'La contraseña debe contener minimo ' + PASSWORD_MIN_LENGTH + ' caracteres'),
    confirmPassword: Yup.string().oneOf([Yup.ref('password'),null], 'Debe coincidir con la contraseña.').required('Se requiere volver a escribir la contraseña.'),
    //Second step fields
    dni: Yup.string().required('El DNI es necesario').matches(/^\d{8}[A-Za-z]$/, 'DNI formato no válido.').test('valid-dni', 'La letra del DNI no es válida para este número.', (value) => {
    ...
    name: Yup.string().required('Se requiere un nombre.').max(NAME_MAX_LENGTH,'El nombre puede contener máximo '+ NAME_MAX_LENGTH +' caracteres.'),
    firstSurname: Yup.string().required("Se requiere el primer apellido.").max(FIRSTSURNAME_MAX_LENGTH,'El primer apellido puede contener máximo ' + FIRSTSURNAME_MAX_LENGTH + ' caracteres.'),
    secondSurname: Yup.string().required("Se requiere el segundo apellido.").max(SECONDSURNAME_MAX_LENGTH,'El segundo apellido puede contener máximo ' + SECONDSURNAME_MAX_LENGTH + ' caracteres'),
    gender: Yup.string().oneOf(['male', 'female', 'other']).defined(),
    birthDate: Yup.date().required('Se necesita una fecha de nacimiento.').max(new Date(),'No debe exceder: ' + new Date().toString()),
    //Third step fields
    postalCode: Yup.string().required('Código postal requerido.').max(POSTALCODE_MAX_LENGTH, "No debe exeder " + POSTALCODE_MAX_LENGTH + " caracteres."),
    ...
    confidencialityTerms: Yup.boolean().required('Se requeire aceptar los términos de confidencialidad.').isTrue('Para registrarse hay que aceptar los términos.')
